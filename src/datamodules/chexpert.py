...
class CheXpertDataset(Dataset):
    ...
    def __init__(self,
                data_dir="data/",
                image_root_path="CheXpert-v1.0-pad224/",
                # image_size=224,
                class_index=-1, 
                use_frontal=True,
                use_upsampling=True,
        ...
        self.df = pd.read_csv(os.path.join(data_dir+image_root_path, csv_path))
        if use_frontal:
        ...
        self._images_list =  [data_dir+path for path in self.df['Path'].tolist()]
        if class_index != -1:
    ...
    def __getitem__(self, idx):
        ...
        image = np.array(Image.open(self._images_list[idx]).convert("RGB"))
        ...
        if self.class_index != -1: # multi-class mode
            label = np.array(self.targets[idx]).reshape(-1).astype(np.int32)
        else:
            label = np.array(self.targets[idx]).reshape(-1).astype(np.int32)
...
if __name__ == '__main__':
    ...
    traindSet = CheXpertDataset(mode='train', class_index=-1)
    testSet =  CheXpertDataset(mode='valid', class_index=-1)
